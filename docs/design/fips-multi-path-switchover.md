...
> (`0.6.0-dev`) and a set of decisions taken in conversation. The decisions
> are recorded as decisions, with the alternatives that lost and why.
>
> All seven steps of "Order of work" landed as seven commits on
> 2026-09-11/12 (`71825007` … `e4b14958`, plus `762ff75a` (review follow-ups) and `4e709e57` (chaos scenario)), each green. Deviations from the
...
> - **Selection also runs from the fast tick** right after heartbeats, not
>   only from the 1 s maintenance tick, so a `Suspect` mark is acted on
...
>   (`DEAD_PATH_GRACE_MS`), pruned on the heartbeat sweep.
...
>
> Additions after review (2026-09-12): probes and acks carry the sender's
> per-path id, and a `PathClose` (0x54, names the receiver's id) tells the
> peer a path is going on a surviving path. The first probe on a path and
> one a minute after are padded to the link MTU (the "data-sized probes"
> option under "Untrusted standby media"; wireless is *not* backup by
> default). Heartbeat interval and echo timeout stretch with each path's
> own round trip, in place of a per-transport interval knob. Defaults
> lean responsive: `K` 1.3, `D` 2 s, `N` 2, 200 ms, timeout 2 × fast.
>
> Nothing in "Calibration" has been measured: those defaults are chosen,
> not derived. The open questions below are still open; the code takes the first
> option of each where one had to be taken (sticky fail-back via `K`,
> every transport probed, wireless not `backup` by default, 250 ms
> while data flows, probe-not-`Suspect`, sequenced ack).
>
> This is the "hold both links" option from
> [fips-dual-path-link-flap.md](fips-dual-path-link-flap.md), worked out. It
> supersedes that note's option 1 (a configured transport prior): once both
> paths are measured a prior is not needed, and the reasons a prior was
> attractive turned out to be reasons against it. See "Why not tiers".
...
Two new inner link-message types next to `0x51 Heartbeat`
(`src/node/dataplane/dispatch.rs:24-61`):

```
0x52 PathProbe  { probe_id: u32, remote_active: bool }
0x53 PathAck    { probe_id: u32, remote_active: bool }
```
...
**Discovery gate.** The beacon gate at `src/node/lifecycle/mod.rs:840`
(`active_peer_link_is_live`) currently skips a live peer entirely. It
becomes: live peer, candidate transport not in the path set → send
`PathProbe` on that transport instead of dialling. The cross-connection path
in `promote_connection` stays for the cases it still owns: a remote epoch
change (restart), and a peer that does not yet know us.
...
Heartbeats become per path and adaptive: fast on a path (order of 250 ms)
if **either** side's active path is this one, slow otherwise (today's
`heartbeat_interval_secs`). The peer's `remote_active` bit says which, so
...
spans the gap and produces one etx spike; `link_cost()` must read the
smoothed value (§8) so that spike does not reach the tree.

**Airtime.** Fifty peers at 250 ms plus acks is about 400 small frames per
...
the link. The 250 ms cadence is for an *idle* active path; what it drops to
while data is flowing is an open item.
...
Wire: two inner message types, `0x52` and `0x53`, five bytes of payload
each. The per-path heartbeat of §7 **is** a `PathProbe` whose `probe_id` is
the path's sequence number, and `PathAck` is its echo, so the sequenced ack
adds no third type; `0x51` stays as the bare per-peer heartbeat for old
nodes. No header change, no handshake change, no index change. Old nodes
drop unknown inner types at `debug` with no misbehaviour accounting
(`src/node/dataplane/dispatch.rs:63`), so a probe sent to an old node is
harmless and simply never acked: the path stays `Probing` and is never
eligible.
...
node:
  path:
    switch_margin: 1.5          # K
    switch_dwell_secs: 5        # D
    min_samples: 3              # N
    active_heartbeat_ms: 250
```

Control: `fipsctl path show <peer>`, `fipsctl path pin|unpin <peer> <transport>`.
...
Steps 1–2 are safe to land on `master` ahead of the rest.

---

## Calibration

`K`, `D`, `N` and the active heartbeat interval are not to be picked by
reasoning; the prior note's warning stands. `parent_hysteresis`,
`hold_down_secs` and `flap_dampening_secs` were all needed before the tree
stopped oscillating, and their values do not transfer to path space. Extend
`testing/chaos/scenarios/ethernet-churn` to carry traffic over two paths
with one flapping, and set the defaults from that distribution.
...
they need a decision from the operator side of the design.
...
§7 puts the active-path heartbeat at about 250 ms so a dead path is noticed
...
path, which may not be this one. Options: keep 250 ms and accept the
