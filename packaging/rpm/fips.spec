...
if [ $1 -eq 0 ]; then
    ...
    # one file per backend it picks between. fips-dns-teardown removes all four
    # on ExecStop, and %%systemd_preun stops the unit before rpm gets here, so
    # this is what catches a host where the unit was not running. rpm has no
    # purge, so this erase branch is the only cleanup that will ever run.
